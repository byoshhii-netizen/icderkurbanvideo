...
router.get('/ara', ac(async (req, res) => {
  ...
  // İsim araması kapalıyken: SADECE numara (telefon) ile arama
  // Metin girişi tamamen reddedilir — etiket araması da yok
  if (!isimleAramaAktif) {
    if (!queryRakamMi) {
    ...
    // Sadece telefon numarası eşleşmesi
    const telefonSonuclar = videolar.filter(v => {
      const dbTel = normalizeTelefon(v.bagisci_telefon);
      if (dbTel && (dbTel === queryTelefon || dbTel.includes(queryTelefon))) return true;
      // Bağışçı etiketlerinde de numara eşleşmesi
      const bagisciEtiketler = [v.etiket1, v.etiket2, v.etiket3, v.etiket4, v.etiket5, v.etiket6, v.etiket7]
        .filter(Boolean);
      return bagisciEtiketler.some(e => {
        const eNorm = normalizeTelefon(String(e));
        return eNorm && (eNorm === queryTelefon || eNorm.includes(queryTelefon));
      });
    });
